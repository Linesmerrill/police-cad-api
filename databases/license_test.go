...
func TestNewLicenseDatabase(t *testing.T) {
	...
	userDB := databases.NewLicenseDatabase(db)

	assert.NotEmpty(t, userDB)
}

func TestLicenseDatabase_FindOne(t *testing.T) {
	...
	dbHelper.(*mocks.DatabaseHelper).
		On("Collection", "licenses").Return(collectionHelper)

	// Create new database with mocked Database interface
	userDba := databases.NewLicenseDatabase(dbHelper)
	...
	assert.Equal(t, &models.License{ID: "mocked-user"}, user)
	assert.NoError(t, err)
}

func TestLicenseDatabase_Find(t *testing.T) {
	...
	srHelperCorrect.(*mocks.SingleResultHelper).
		...
		Return(nil).Run(func(args mock.Arguments) {
		arg := args.Get(0).(*[]models.License)
		*arg = []models.License{{ID: "mocked-user"}}
	})
