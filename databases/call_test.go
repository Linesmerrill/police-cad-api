...
func TestNewCallDatabase(t *testing.T) {
	...
	userDB := databases.NewCallDatabase(db)

	assert.NotEmpty(t, userDB)
}

func TestCallDatabase_FindOne(t *testing.T) {
	...
	dbHelper.(*mocks.DatabaseHelper).
		On("Collection", "calls").Return(collectionHelper)

	// Create new database with mocked Database interface
	userDba := databases.NewCallDatabase(dbHelper)
	...
	assert.Equal(t, &models.Call{ID: "mocked-user"}, user)
	assert.NoError(t, err)
}

func TestCallDatabase_Find(t *testing.T) {
	...
	srHelperCorrect.(*mocks.SingleResultHelper).
		...
		Return(nil).Run(func(args mock.Arguments) {
		arg := args.Get(0).(*[]models.Call)
		*arg = []models.Call{{ID: "mocked-user"}}
	})
