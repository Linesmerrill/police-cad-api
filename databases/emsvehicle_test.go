...
func TestNewEmsVehicleDatabase(t *testing.T) {
	...
	userDB := databases.NewEmsVehicleDatabase(db)

	assert.NotEmpty(t, userDB)
}

func TestEmsVehicleDatabase_FindOne(t *testing.T) {
	...
	dbHelper.(*mocks.DatabaseHelper).
		On("Collection", "emsvehicles").Return(collectionHelper)

	// Create new database with mocked Database interface
	userDba := databases.NewEmsVehicleDatabase(dbHelper)
	...
	assert.Equal(t, &models.EmsVehicle{ID: "mocked-user"}, user)
	assert.NoError(t, err)
}

func TestEmsVehicleDatabase_Find(t *testing.T) {
	...
	srHelperCorrect.(*mocks.SingleResultHelper).
		...
		Return(nil).Run(func(args mock.Arguments) {
		arg := args.Get(0).(*[]models.EmsVehicle)
		*arg = []models.EmsVehicle{{ID: "mocked-user"}}
	})
