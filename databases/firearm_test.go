...
func TestNewFirearmDatabase(t *testing.T) {
	...
	userDB := databases.NewFirearmDatabase(db)

	assert.NotEmpty(t, userDB)
}

func TestFirearmDatabase_FindOne(t *testing.T) {
	...
	dbHelper.(*mocks.DatabaseHelper).
		On("Collection", "firearms").Return(collectionHelper)

	// Create new database with mocked Database interface
	userDba := databases.NewFirearmDatabase(dbHelper)
	...
	assert.Equal(t, &models.Firearm{ID: "mocked-user"}, user)
	assert.NoError(t, err)
}

func TestFirearmDatabase_Find(t *testing.T) {
	...
	srHelperCorrect.(*mocks.SingleResultHelper).
		...
		Return(nil).Run(func(args mock.Arguments) {
		arg := args.Get(0).(*[]models.Firearm)
		*arg = []models.Firearm{{ID: "mocked-user"}}
	})
