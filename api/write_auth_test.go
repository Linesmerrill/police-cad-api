...
func TestRequireWriteAuth(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "browser write from our origin allowed",
			enforce:    "true",
			method:     http.MethodPatch,
			path:       "/api/v1/community/abc",
			headers:    map[string]string{"Origin": "https://www.linespolice-cad.com"},
			wantStatus: http.StatusOK,
			wantPass:   true,
		},
		{
			name:       "browser write via referer allowed",
			enforce:    "true",
			method:     http.MethodPost,
			path:       "/api/v1/community/abc/roles",
			headers:    map[string]string{"Referer": "https://www.linespolice-cad.com/community/abc"},
			wantStatus: http.StatusOK,
			wantPass:   true,
		},
		{
			name:       "write from foreign origin rejected",
			enforce:    "true",
			method:     http.MethodPatch,
			path:       "/api/v1/community/abc",
