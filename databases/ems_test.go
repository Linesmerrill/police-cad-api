...
func TestNewEmsDatabase(t *testing.T) {
	...
	userDB := databases.NewEmsDatabase(db)

	assert.NotEmpty(t, userDB)
}

func TestEmsDatabase_FindOne(t *testing.T) {
	...
	dbHelper.(*mocks.DatabaseHelper).
		On("Collection", "ems").Return(collectionHelper)

	// Create new database with mocked Database interface
	userDba := databases.NewEmsDatabase(dbHelper)
	...
	assert.Equal(t, &models.Ems{ID: "mocked-user"}, user)
	assert.NoError(t, err)
}

func TestEmsDatabase_Find(t *testing.T) {
